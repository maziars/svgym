...
def round_attribute_coords(svg_text, precision=1):
    ...
    """
    # Preserve text geometry exactly: rounding x/y on <text>/<tspan> shifts glyph
    # positions sub-pixel, which changes font hinting and is visible on small/bold
    # text. Mask text elements out, round everything else, then restore verbatim.
    _text_blocks = []
    def _mask_text(m):
        _text_blocks.append(m.group(0))
        return "\x00T%d\x00" % (len(_text_blocks) - 1)
    svg_text = re.sub(r"<text\b.*?</text>", _mask_text, svg_text, flags=re.DOTALL)
    ...
    for _i, _orig in enumerate(_text_blocks):
        svg_text = svg_text.replace("\x00T%d\x00" % _i, _orig)
    return svg_text
